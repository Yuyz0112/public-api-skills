...
|-------|------|----------|-------------|
| `checkout_session` | string | No | The Checkout session that this coupon is applied to, if it is applied to a particular session in payment mode. Will not be present for subscription mode. |
...
| `end` | integer (unix-time) | No | If the coupon has a duration of `repeating`, the date that this discount will end. If the coupon has a duration of `once` or `forever`, this attribute will be null. |
| `id` | string | Yes | The ID of the discount object. Discounts cannot be fetched by ID. Use `expand[]=discounts` in API calls to expand discount IDs in an array. |
...
| `object` | enum: discount | Yes | String representing the object's type. Objects of the same type share the same value. |
| `promotion_code` | any | No | The promotion code applied to create this discount. |
| `source` | [discount_source](discount-source.md) | Yes |  |
| `start` | integer (unix-time) | Yes | Date that the coupon was applied. |
