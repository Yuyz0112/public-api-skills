...
| `amount_type` | enum: fixed, maximum | Yes | The type of amount that will be collected. The amount charged must be exact or up to the value of `amount` param for `fixed` or `maximum` type respectively. Defaults to `maximum`. |
| `end_date` | string | No | Date, in YYYY-MM-DD format, after which payments will not be collected. Defaults to no end date. |
| `payment_schedule` | enum: adhoc, annual, daily... | Yes | The periodicity at which payments will be collected. Defaults to `adhoc`. |
