...
**Operation ID:** `GetIssuingTransactions`
...
|------|------|------|----------|-------------|
| `card` | query | string | No | Only return transactions that belong to the given card. |
| `cardholder` | query | string | No | Only return transactions that belong to the given cardholder. |
...
| `type` | query | enum: capture, refund | No | Only return transactions that have the given type. One of `capture` or `refund`. |
