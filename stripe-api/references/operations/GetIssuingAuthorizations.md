...
**Operation ID:** `GetIssuingAuthorizations`
...
| `created` | query | any | No | Only return authorizations that were created during the given date interval. |
...
| `status` | query | enum: closed, expired, pending... | No | Only return authorizations with the given status. One of `pending`, `closed`, or `reversed`. |
