# GET /admin/api/2020-01/redirects/count.json

**Resource:** [online-store/redirect](../resources/online-store-redirect.md)
**Retrieves a count of URL redirects**
**Operation ID:** `deprecated_202001_get_redirects_count`

https://shopify.dev/docs/admin-api/rest/reference/online-store/redirect#count-2020-01

## Parameters

| Name | In | Type | Required | Description |
|------|------|------|----------|-------------|
| `path` | query | any | No | Count redirects with given path. |
| `target` | query | any | No | Count redirects with given target. |

## Responses

| Status | Description |
|--------|-------------|
| 200 |  |

