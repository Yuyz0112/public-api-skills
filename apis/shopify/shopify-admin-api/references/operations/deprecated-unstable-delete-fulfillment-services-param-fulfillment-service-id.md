# DELETE /admin/api/unstable/fulfillment_services/{fulfillment_service_id}.json

**Resource:** [shipping-and-fulfillment/fulfillmentservice](../resources/shipping-and-fulfillment-fulfillmentservice.md)
**Operation ID:** `deprecated_unstable_delete_fulfillment_services_param_fulfillment_service_id`

https://shopify.dev/docs/admin-api/rest/reference/shipping-and-fulfillment/fulfillmentservice#destroy-unstable

## Parameters

| Name | In | Type | Required | Description |
|------|------|------|----------|-------------|
| `fulfillment_service_id` | path | string | Yes | fulfillment_service_id |

## Responses

| Status | Description |
|--------|-------------|
| 200 |  |

