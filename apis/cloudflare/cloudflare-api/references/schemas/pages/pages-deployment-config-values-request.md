...
| `always_use_latest_compatibility_date` | boolean | No | Whether to always use the latest compatibility date for Pages Functions. |
...
| `build_image_major_version` | integer | No | The major version of the build image to use for Pages Functions. |
| `compatibility_date` | string | No | Compatibility date used for Pages Functions. |
| `compatibility_flags` | string[] | No | Compatibility flags used for Pages Functions. |
...
| `env_vars` | object | No | Environment variables used for builds and Pages Functions. |
| `fail_open` | boolean | No | Whether to fail open when the deployment config cannot be applied. |
...
| `usage_model` | enum: standard, bundled, unbound | No | The usage model for Pages Functions. |
