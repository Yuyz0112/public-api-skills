...
**Operation ID:** `radar-get-leaked-credential-checks-timeseries-group`
...
|------|------|------|----------|-------------|
| `dimension` | path | enum: COMPROMISED, BOT_CLASS | Yes | Specifies the attribute by which to group the results. |
...
| `checkResult` | query | string[] | No | Filters results by leaked credential check result. |
