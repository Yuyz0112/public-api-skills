...
| `instances_url` | string (uri) | No |  |
| `most_recent_instance` | object | No |  |
| `number` | integer | Yes | The code scanning alert number. |
| `rule` | object | Yes |  |
| `state` | enum: fixed | Yes | State of a code scanning alert. Events for alerts found outside the default branch will return a `null` value until they are dismissed or fixed. |
| `tool` | object | Yes |  |
| `url` | string (uri) | Yes |  |
