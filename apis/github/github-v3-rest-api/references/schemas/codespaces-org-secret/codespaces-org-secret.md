...
| `updated_at` | string (date-time) | Yes | The date and time at which the secret was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ. |
| `visibility` | enum: all, private, selected | Yes | The type of repositories in the organization that the secret is visible to |
| `selected_repositories_url` | string (uri) | No | The API URL at which the list of repositories this secret is visible to can be retrieved |
