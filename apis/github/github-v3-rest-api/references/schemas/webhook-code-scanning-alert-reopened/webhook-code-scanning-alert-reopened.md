...
| `commit_oid` | string | Yes | The commit SHA of the code scanning alert. When the action is `reopened_by_user` or `closed_by_user`, the event was triggered by the `sender` and this value will be empty. |
| `enterprise` | [enterprise-webhooks](enterprise-webhooks.md) | No |  |
| `installation` | [simple-installation](simple-installation.md) | No |  |
| `organization` | [organization-simple-webhooks](organization-simple-webhooks.md) | No |  |
| `ref` | string | Yes | The Git reference of the code scanning alert. When the action is `reopened_by_user` or `closed_by_user`, the event was triggered by the `sender` and this value will be empty. |
...
| `dismissed_reason` | string | Yes | The reason for dismissing or closing the alert. Can be one of: `false positive`, `won't fix`, and `used in tests`. |
...
| `instances_url` | string | No |  |
| `most_recent_instance` | object | No |  |
| `number` | integer | Yes | The code scanning alert number. |
| `rule` | object | Yes |  |
| `state` | enum: open, dismissed, fixed | Yes | State of a code scanning alert. Events for alerts found outside the default branch will return a `null` value until they are dismissed or fixed. |
| `tool` | object | Yes |  |
| `updated_at` | string | No |  |
| `url` | string (uri) | Yes |  |
| `dismissal_approved_by` | any | No |  |
