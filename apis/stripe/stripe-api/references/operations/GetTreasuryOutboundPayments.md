...
|------|------|------|----------|-------------|
| `created` | query | any | No | Only return OutboundPayments that were created during the given date interval. |
| `customer` | query | string | No | Only return OutboundPayments sent to this customer. |
...
| `status` | query | enum: canceled, failed, posted... | No | Only return OutboundPayments that have the given status: `processing`, `failed`, `posted`, `returned`, or `canceled`. |
