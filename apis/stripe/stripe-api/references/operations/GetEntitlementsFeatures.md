...
**List all features**
**Operation ID:** `GetEntitlementsFeatures`
...
|------|------|------|----------|-------------|
| `archived` | query | boolean | No | If set, filter results to only include features with the given archive status. |
...
| `lookup_key` | query | string | No | If set, filter results to only include features with the given lookup_key. |
