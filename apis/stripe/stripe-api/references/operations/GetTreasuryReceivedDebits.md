...
**Operation ID:** `GetTreasuryReceivedDebits`
...
| `financial_account` | query | string | Yes | The FinancialAccount that funds were pulled from. |
...
| `status` | query | enum: failed, succeeded | No | Only return ReceivedDebits that have the given status: `succeeded` or `failed`. |
