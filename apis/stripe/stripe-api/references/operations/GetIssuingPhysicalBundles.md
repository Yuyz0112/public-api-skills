...
**Operation ID:** `GetIssuingPhysicalBundles`
...
| `status` | query | enum: active, inactive, review | No | Only return physical bundles with the given status. |
| `type` | query | enum: custom, standard | No | Only return physical bundles with the given type. |
