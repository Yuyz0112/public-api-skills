...
| `finished` | string (date-time) | No | A timestamp recording when the task was finished. |
| `id` | string | Yes | The ID of the task. |
| `lastUpdate` | string (date-time) | Yes | A timestamp recording when the task progress was last updated. |
...
| `started` | string (date-time) | No | A timestamp recording when the task was started. |
| `status` | enum: ENQUEUED, RUNNING, COMPLETE... | Yes | The status of the task. |
| `submitted` | string (date-time) | No | A timestamp recording when the task was submitted. |
